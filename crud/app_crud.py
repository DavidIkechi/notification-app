...
def update_client(db: Session, client_key, update_client_data):
    try:
        ...
        if get_client is None:
            return exceptions.bad_request_error(detail="Client with such key does not exists")   
        # update client_field
...
def update_client_key(db: Session, old_key, new_key):
    try:
        ...
        if old_key == new_key.client_key:
            return exceptions.bad_request_error(detail="Old and new keys must be not be the same")
